...
- `OPENMSX_EXECUTABLE` should point to the openMSX binary if it's not in your system's PATH. The default is `openmsx.exe` on Windows and `openmsx` on Linux/macOS. For example, on Windows it might be `C:\Program Files\openMSX\openmsx.exe`, and on Linux/Mac it could be `/usr/local/bin/openmsx` or `~/bin/openmsx`. In JSON configuration files, backslashes must be escaped: `"C:\\Program Files\\openMSX\\openmsx.exe"`.
